      ...
      fetch('/api/agency/users')
        ...
        .then(data => {
          if (Array.isArray(data)) {
            // FILTER: Vylúčiť "nepriraditeľné" oddelenia
            // Management, Podporné oddelenia (HR, Finance...)
            const FORBIDDEN_DEPTS = ["Management", "Podporné oddelenia", "Vedenie agentúry", "Support"]
            // Poznámka: "Vedenie agentúry" je v tvojom zozname, ja som to v UsersSettings nazval "Management"
            // Pre istotu filtrujem oboje verzie + Finance ak by bolo explitne

            const filtered = data.filter((u: any) => {
              const dept = u.department || ""
              // Ak je to Management, Podporné oddelenia alebo Financie -> FALSE
              if (dept.includes("Management") || dept.includes("Vedenie")) return false
              if (dept.includes("Podporné") || dept.includes("Support")) return false
              if (dept.includes("Finance") || dept.includes("Financie")) return false
              return true
            })
            setUsers(filtered)
          } else {
